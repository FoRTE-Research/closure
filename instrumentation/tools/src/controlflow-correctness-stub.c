...
char* coverage_dump_fname              = NULL;
FILE* coverage_dump_file               = NULL;
char* closure_edge_dump                = NULL;
int closure_edge_coverage_data_written = 0x0;

void dump_coverage_data() {
    if (closure_edge_dump != NULL && closure_edge_coverage_data_written > 0) {
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 0; i < closure_iteration_count; ++i) {
        ...
        if (i == closure_iteration_count - 1) {
            if (getenv("CLOSURE_COVERAGE_DUMP")) {
                unlink(getenv("CLOSSURE_COVERAGE_DUMP"));
                coverage_dump_file = fopen(getenv("CLOSURE_COVERAGE_DUMP"), "w");
                closure_edge_dump  = (char*)calloc(1, 10000);
            }

            else {
                coverage_dump_file = stdout;
            }
        }
    ...
    printf("\n---Done with Controlflow Correctness Testing---\n");
