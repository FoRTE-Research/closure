...
int check_and_restore_global_sections(char *closure_global_section_addr, char *closure_global_section_copy, int closure_global_section_size)
{
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < 1; ++i)
    {
        ...
        int num_modified = check_and_restore_global_sections(closure_global_section_addr, closure_global_section_copy, closure_global_section_size);
    
        printf("Global Section size = %d\n Number of global bytes modified = %d", closure_global_section_size, num_modified);
    }
