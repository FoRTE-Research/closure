...
def process_queue_dir(corpus_dir, cmdline, cache_dir=None, result_csv=None):
    ...
    for f in corpus:
        ...
        if (cache_dir) is None:
            ...
            COVERAGE_FILE_PATH = os.path.join(cache_dir, "coverage" + str(get_queue_id(f)))
            if os.path.exists(COVERAGE_FILE_PATH) == False:
                env[COVERAGE_DUMP_FILE] = COVERAGE_FILE_PATH
                run = subprocess.run(command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=env)

        if os.path.exists(COVERAGE_FILE_PATH):
            unique_edges, total_edges = process_coverage_dump_file(COVERAGE_FILE_PATH)
...
def main():
    ...
    if IS_SINGLE_INPUT:
        ...
        run = subprocess.run(command,  capture_output=False, text=False, env=env)
        ...
        if os.path.exists(COVERAGE_FILE_PATH):
            ...
            print(f"{bcolors.WARNING} No Coverage Dump File generated. Is it compiled with trace-pc-guard? {bcolors.ENDC}")
