...
def main():
    ...
    parser.add_argument("--addr" , dest="target_global_section_addr", required=True)
    ...
    if IS_SINGLE_INPUT:
        ...
        if os.path.exists(BITMAP_FILE_PATH):
            num_bytes_modified, max_distance = process_global_bitmap_file(BITMAP_FILE_PATH)
            print(f"Global section size {args.target_global_section_size}")
            print(f"Number of global section bytes modified {num_bytes_modified}")
        ...
        for f in os.listdir(corpus_dir):
            ...
            if os.path.isfile(fname):
                ...
                if os.path.exists(BITMAP_FILE_PATH):
                    num_bytes_modified, max_distance = process_global_bitmap_file(BITMAP_FILE_PATH)
                    max_num_bytes_modified = num_bytes_modified if (num_bytes_modified > max_num_bytes_modified) else max_num_bytes_modified
        ...
        print(f"Global Section size {args.target_global_section_size}")
        print(f"Max number of Global modified bytes is {max_num_bytes_modified}\n")
        print(f"Average number of bytes modified is { int(sum(num_bytes_modified_list)/len(num_bytes_modified_list))}")
